...
-- 06_procedures.sql - Business Workflow Engine (CORRECTED VERSION)

DROP PROCEDURE IF EXISTS create_pickup_request(INT, DATE, TEXT, TEXT);
DROP PROCEDURE IF EXISTS add_item_to_pickup(INT, INT, TEXT, VARCHAR, DECIMAL, JSONB);
DROP PROCEDURE IF EXISTS record_item_weight(INT, VARCHAR, DECIMAL, VARCHAR, TEXT);
DROP PROCEDURE IF EXISTS assign_pickup_to_staff(INT, INT, INT, INT, TIMESTAMP);
DROP PROCEDURE IF EXISTS complete_pickup_collection(INT);
DROP PROCEDURE IF EXISTS process_payment(INT, VARCHAR, VARCHAR);
DROP PROCEDURE IF EXISTS create_recycling_batch(INT, VARCHAR, TEXT);
DROP PROCEDURE IF EXISTS add_item_to_batch(INT, INT);
DROP PROCEDURE IF EXISTS start_batch_processing(INT);
DROP PROCEDURE IF EXISTS complete_batch_processing(INT, DECIMAL);
...
BEGIN
   ...
   RAISE NOTICE 'Batch % completed with %%% recovery rate', 
             p_batch_id, 
             p_recovery_rate_percentage;
END;

$$;

-- Comments explaining procedure design:
-- 1. OUT parameters now come LAST in parameter list (PostgreSQL requirement)
-- 2. Each procedure represents a complete business workflow
-- 3. All validations happen inside procedures
-- 4. Multiple tables updated in single transaction
-- 5. OUT parameters return generated IDs
-- 6. RAISE EXCEPTION rolls back transaction on error
-- 7. RAISE NOTICE provides feedback on success
-- 8. Status transitions validated
-- 9. Referential integrity maintained
-- 10. Procedures call functions for calculations
